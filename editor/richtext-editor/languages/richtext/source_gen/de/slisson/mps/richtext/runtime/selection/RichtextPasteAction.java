  ...
  public void execute(final EditorContext context) {
    ...
    boolean allAreIWords = ListSequence.fromList(nodes).foldLeft(true, new ILeftCombinator<SNode, Boolean>() {
      public Boolean combine(Boolean s, SNode it) {
        return s && SNodeOperations.isInstanceOf(it, "de.slisson.mps.richtext.structure.IWord");
      }
    });

    if (ListSequence.fromList(nodes).isNotEmpty() && allAreIWords) {
      final Wrappers._T<Integer> caretPos = new Wrappers._T<Integer>(check_j8mhr4_a0a0e0c(SNodeOperations.as(ListSequence.fromList(nodes).last(), "de.slisson.mps.richtext.structure.Word")));
